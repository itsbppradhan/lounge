...
export function PlaceholdersAndVanishInput({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    vanishAndSubmit();
    if (onSubmit) {
      onSubmit(e); // Ensure function call is made
    }
  };
  return (
    <form
      className={cn(
        "w-full relative max-w-xl mx-auto bg-zinc-800/30 border border-zinc-300/30 backdrop-blur-xl h-12 rounded-full overflow-hidden shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),_0px_1px_0px_0px_rgba(25,28,33,0.02),_0px_0px_0px_1px_rgba(25,28,33,0.08)] transition duration-200",
        value && "bg-gray-500/30"
      )}
      ...
      <input
        onChange={(e) => {
          if (!animating) {
            ...
            if (onChange) {
              onChange(e); // Ensure function call is made
            }
          }
        }}
        ...
        <AnimatePresence mode="wait">
          {!value && (
            <motion.p
              ...
              className="text-zinc-200/50 text-sm sm:text-base font-normal pl-4 sm:pl-12 text-left w-[calc(100%-2rem)] truncate"
            >
